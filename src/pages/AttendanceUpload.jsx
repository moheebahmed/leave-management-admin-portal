...
const computeRow = (row) => {
  ...
  const late =
    checkInMins != null && onDutyMins != null && checkInMins > onDutyMins
      ? checkInMins - onDutyMins
      : null;
  const early =
    checkOutMins != null && offDutyMins != null && checkOutMins < offDutyMins
      ? offDutyMins - checkOutMins
      : null;
  const work =
    checkInMins != null && checkOutMins != null && checkOutMins > checkInMins
      ? checkOutMins - checkInMins
      : null;
  const ot =
    checkOutMins != null && offDutyMins != null && checkOutMins > offDutyMins
      ? checkOutMins - offDutyMins
      : null;
...
const mapRow = (row) => ({
  employee_code: String(
    row["Emp No."] || row["employee_code"] || row["Emp No"] || "",
  ).trim(),
  date: excelDateToString(row["Date"] || row["date"]),
  check_in:
    toTimeString(row["Clock In"] || row["check_in"] || row["CheckIn"]) || null,
  check_out:
    toTimeString(row["Clock Out"] || row["check_out"] || row["CheckOut"]) ||
    null,
  on_duty:
    toTimeString(row["On duty"] || row["on_duty"] || row["On Duty"]) || null,
  off_duty:
    toTimeString(row["Off duty"] || row["off_duty"] || row["Off Duty"]) || null,
});
...
const AttendanceUpload = () => {
  ...
  const handleUpload = async () => {
    ...
    setLoading(true);
    try {
      await axios.post(`${API_BASE_URL}/attendance/upload`, payload, {
    ...
    <div className="animate-fade-slide space-y-5">
      ...
      <div>
        ...
        <p className="page-subtitle font-semibold text-[rgb(173,173,173)]">
          Upload attendance Excel file — it will be parsed and sent to the
          server
        </p>
      </div>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-slate-500">
                  {(file.size / 1024).toFixed(1)} KB · {payload.length} records
                  parsed
                </p>
              </div>
            </div>
              ...
              <thead>
                <tr className="bg-[#000000]">
                  {[
                    ...
                    <th
                      key={h}
                      className="table-th text-[11px] font-semibold text-[rgb(173,173,173)] whitespace-nowrap"
                    >
                      {h}
                    </th>
                  ))}
              ...
              <tbody>
                {preview.map((row, i) => (
                  ...
                  >
                    ...
                    {/* On Duty */}
                    <td className="table-td text-slate-400 text-xs font-mono whitespace-nowrap">
                      {row.on_duty || "—"}
                    </td>
                    ...
                    <td className="table-td text-xs font-mono whitespace-nowrap">
                      {row.late_minutes ? (
                        <span className="text-red-400 font-semibold bg-red-500/10 px-1.5 py-0.5 rounded">
                          {row.late_minutes}
                        </span>
                      ) : (
                    ...
                    <td className="table-td text-xs font-mono whitespace-nowrap">
                      {row.early_minutes ? (
                        <span className="text-green-400 font-semibold bg-green-500/10 px-1.5 py-0.5 rounded">
                          {row.early_minutes}
                        </span>
                      ) : (
                        <span className="text-slate-600">—</span>
