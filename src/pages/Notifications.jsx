...
const Notifications = () => {
  ...
  useEffect(() => {
    axios
      .get(`${API_BASE_URL}/employees/notifications`, {
  ...
  const markAllRead = async () => {
    await axios.put(
      `${API_BASE_URL}/employees/notifications/read-all`,
      {},
      { headers: getAuthHeaders() },
    );
  ...
  const markRead = async (id) => {
    await axios.put(
      `${API_BASE_URL}/employees/notifications/${id}/read`,
      {},
      { headers: getAuthHeaders() },
    );
      ...
      <div className="card-base overflow-hidden divide-y divide-border">
        {loading ? (
          ...
          notifications.map((n) => (
            ...
            >
              <div
                className={`w-2 h-2 rounded-full mt-1.5 shrink-0 ${!n.is_read ? "bg-accent" : "bg-transparent"}`}
              />
              <div className="flex-1 min-w-0">
                <p
                  className={`text-sm font-medium ${!n.is_read ? "text-slate-100" : "text-slate-400"}`}
                >
                  {n.title}
                </p>
