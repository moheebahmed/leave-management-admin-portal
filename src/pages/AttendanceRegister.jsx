    ...
    <div className="animate-fade-slide space-y-5">
      ...
      <div className="flex flex-wrap items-center gap-3 text-xs">
        {[
          ...
          <span
            key={l.label}
            className={`px-2.5 py-1 rounded-full border font-semibold ${l.color}`}
          >
            {l.label}
          </span>
        ))}
              ...
              <thead>
                ...
                ${isWeekend(d) ? "text-slate-600" : "text-[rgb(173,173,173)]"}`}
                    >
                      ...
                      {/* show weekday only on big screens */}
                  ...
                  return (
                    <tr key={emp.employee_id} className="table-row-hover">
                      ...
                      {days.map((d) => {
                        ...
                        return (
                          <td key={d} className="table-td text-center px-1">
                            <span
                              className={`inline-flex items-center justify-center w-6 h-6 rounded text-[10px] font-bold ${cell.bg} ${cell.text}`}
                            >
                              {cell.label}
                            </span>
                          </td>
      ...
      {!error && employees.length === 0 && (
        <div className="card-base flex items-center justify-center py-16 text-slate-500 text-sm">
          ...
          create records for {MONTHS[month]} {year}.
        </div>
      )}
    </div>
  );
